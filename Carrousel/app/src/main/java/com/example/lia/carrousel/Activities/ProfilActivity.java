...
public class ProfilActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter = new MyAdapter(TITLES, ICONS, "", "", R.mipmap.carousel);       // Creating the Adapter of MyAdapter class(which we are going to see in a bit)
        ...
        phoneNumber.setText(phoneNumber.getText().toString() + demandUser.getPhoneNumber());
    }

    public void confirmer(View v){
        DBDemande dbDemande = new DBDemande(this.getBaseContext());
        if(user.getType().equals(User.UserEntry.GARDIEN_TYPE)){
            CarrouselManagementSystem.confirmDemand(user.getUsername(),demandUser.getUsername(),dbDemande);
        }else{
            CarrouselManagementSystem.confirmDemand(demandUser.getUsername(),user.getUsername(),dbDemande);
        }
    ...
    public void refuser(View v){
        DBDemande dbDemande = new DBDemande(this.getBaseContext());
        if(user.getType().equals(User.UserEntry.GARDIEN_TYPE)){
            CarrouselManagementSystem.refuseDemand(user.getUsername(),demandUser.getUsername(),dbDemande);
        }else{
            CarrouselManagementSystem.refuseDemand(demandUser.getUsername(),user.getUsername(),dbDemande);
        }
        intent = new Intent(this,MainPageActivity.class);
        intent.putExtra("user",user);
        startActivity(intent);
